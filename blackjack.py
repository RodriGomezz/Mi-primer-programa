...
class Card:
    ...
    def __init__(self,number, suit):
        ...
        self.tk_img = ImageTk.PhotoImage(
            Image.open(f"{self.card_image_path}"
                       f"{self.number_name_mapping.get(self.number, self.number)}_of_"
                       f"{self.suit}.png").resize((103, 150), Image.ANTIALIAS))
...
class BlackjackUI:
    ...
    def end_game(self):
        ...
        if self.game.does_player_win():
            showinfo("Resultado partida", "SII PADREEE!\n¡Has ganao!")
        else:
            showinfo("Resultado partida", "Donde te sentaste!\nperdiste")
